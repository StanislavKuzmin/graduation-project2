...
public interface DishDateRepository extends JpaRepository<DishDate, DishDateId> {
    ...
    @Transactional
    @Modifying
    @Query("DELETE FROM DishDate dd WHERE dd.id=:id")
    ...
    @Query("SELECT new ru.javaops.topjava2.to.DishTo(d.dish.id, d.dish.name, d.dish.price, d.dish.calories, d.id.date)" +
            " FROM DishDate d WHERE d.dish.restaurantId =:restaurantId AND d.id.date =:date" +
    ...
    @EntityGraph(attributePaths = {"dish"}, type = EntityGraph.EntityGraphType.LOAD)
    @Query("SELECT new ru.javaops.topjava2.to.DishTo(d.dish.id, d.dish.name, d.dish.price, d.dish.calories, d.id.date)" +
            " FROM DishDate d WHERE d.dish.restaurantId =:restaurantId AND d.id.date =current_date" +
            " ORDER BY d.dish.price DESC")
    List<DishTo> getRestaurantMenuToday(@Param("restaurantId") int restaurantId);

    default DishDate get(DishDateId id) {
        return findById(id)
                .orElseThrow(() -> new NotFoundException("Dish with id=" + id.getDishId()
                        + " not found in menu for the date " + id.getDate()));
    }
}
